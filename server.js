    ...
    try {
        ...
        bcrypt.genSalt(10, (err, salt) => {
            if (err) {
                ...
                return res.status(500).send({ message: 'An error occurred during registration. Please try again later.' });
            }
            ...
            bcrypt.hash(user.password, salt, async (err, hpass) => {
                ...
                try {
                    ...
                    res.status(201).send({ message: "Üyelik Oluşturuldu!" });
                } catch (err) {
...
// endpoint for forgot password and reset password
...
app.post('/forgotpassword', (req, res) => {
    ...
    .then(user => {
        ...
        const token = jwt.sign({ id: user._id }, "nutritionApp", { expiresIn: "1d" });
...
app.post('/resetpassword/:id/:token', (req, res) => {
    ...
    jwt.verify(token, "nutritionApp", (err, decoded) => {
        if (err) {
            ...
            bcrypt.hash(password, 10)
                .then(hash => {
                    userModel.findByIdAndUpdate(id, { password: hash })
                        ...
                        .catch(err => res.status(500).json({ Status: "Error updating password" }));
                })
                .catch(err => res.status(500).json({ Status: "Error hashing password" }));
        }
    });
});
...
app.get("/foods/:name", verifyToken, async (req, res) => {
    try {
        // Convert Turkish characters in search query to case-insensitive regex
        const regexPattern = turkishInsensitiveRegex(req.params.name);
...
app.get("/userfoods/:name", verifyToken, async (req, res) => {
    try {
        // Convert Turkish characters in search query to case-insensitive regex
        const regexPattern = turkishInsensitiveRegex(req.params.name);

        // Fetch user-specific food items
